...
main()
{
  ...
  show_network_bandwith=$(get_tmux_option "@dracula-show-network_bandwith" true)
  show_ip_network=$(get_tmux_option "@dracula-show-ip_network" true)
...
#  show_right_sep=$(get_tmux_option "@dracula-show-right-sep" )
  ...
  show_refresh=$(get_tmux_option "@dracula-refresh-rate" 60)
  ...
  if $show_powerline; then
      ...
      tmux set-option -g status-left "#[bg=${pink},fg=${dark_gray}]#{?client_prefix,#[bg=${yellow}],} ${left_icon} #[fg=${gray},bg=${pink}]#{?client_prefix,#[fg=${yellow}],}${left_sep}"
	 ...
	 tmux set-option -ga status-right "#[fg=${pink},bg=${powerbg},nobold,nounderscore,noitalics] ${right_sep}#[fg=${dark_gray},bg=${cyan}] #($current_dir/gpu_usage.sh)"
	 powerbg=${cyan}
      fi

      if $show_network; then # network
        tmux set-option -ga status-right "#[fg=${cyan},bg=${powerbg},nobold,nounderscore,noitalics] ${right_sep}#[fg=${dark_gray},bg=${orange}] #($current_dir/network.sh)"
        powerbg=${pink}
      fi

      if $show_network_bandwith; then # network_bandwith
        tmux set-option -ga status-left "#[fg=${orange},bg=${powerbg},nobold,nounderscore,noitalics] ${right_sep}#[fg=${dark_gray},bg=${cyan}] #($current_dir/network_bandwith.sh)"
        powerbg=${cyan}
      fi

      if $show_ip_network; then # ip_network
        tmux set-option -ga status-left "#[fg=${cyan},bg=${powerbg},nobold,nounderscore,noitalics] ${left_sep}#[fg=${dark_gray},bg=${pink}] #($current_dir/ip_network.sh)"
        powerbg=${pink}
      fi

      if $show_weather; then # weather
        tmux set-option -ga status-left "#[fg=${orange},bg=${powerbg},nobold,nounderscore,noitalics] ${left_sep}#[fg=${dark_gray},bg=${orange}] #(cat $current_dir/../data/weather.txt)"
        powerbg=${orange}
      fi
      ...
      tmux set-window-option -g window-status-current-format "#[fg=${gray},bg=${pink}]${left_sep}#[fg=${black},bg=${pink}] #I #W${current_flags} #[fg=${dark_purple},bg=${gray}]${right_sep}"
  ...
  # Non Powerline Configuration
    ...
    tmux set-option -g  status-right ""
      ...
      if $show_battery; then # battery
        tmux set-option -g  status-left "#[fg=${dark_gray},bg=${pink}] #($current_dir/battery.sh) "
      fi
      if $show_ram_usage; then
	tmux set-option -ga status-right "#[fg=${dark_gray},bg=${cyan}] #($current_dir/ram_info.sh) "
      ...
      if $show_cpu_usage; then
	tmux set-option -ga status-right "#[fg=${dark_gray},bg=${pink}] #($current_dir/cpu_info.sh) "
      fi
	...
	tmux set-option -ga status-right "#[fg=${dark_gray},bg=${pink}] #($current_dir/gpu_usage.sh) "
      ...
      if $show_network_bandwith; then # network
        tmux set-option -ga status-right "#[fg=${dark_gray},bg=${pink}] #($current_dir/network_bandwith.sh) "
      fi
      ...
      if $show_time; then
        if $show_day_month && $show_military ; then # military time and dd/mm
          tmux set-option -ga status-left "#[fg=${white},bg=${dark_purple}] %a %d/%m %R ${timezone} "
        elif $show_military; then # only military time
          tmux set-option -ga status-left "#[fg=${white},bg=${dark_purple}] %a %m/%d %R ${timezone} "
        elif $show_day_month; then # only dd/mm
          tmux set-option -ga status-left "#[fg=${white},bg=${dark_purple}] %a %d/%m %I:%M %p ${timezone} "
        else
          tmux set-option -ga status-left "#[fg=${white},bg=${dark_purple}] %a %m/%d %I:%M %p ${timezone} "
        fi
      fi
